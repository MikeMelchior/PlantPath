...
- **Webhook handler design: Svix signature verification, idempotent upsert, semantic return codes.** The Clerk webhook at `/api/webhooks/clerk` verifies every request via Svix before trusting the payload (the endpoint is public-facing, so unsigned requests are rejected with 401). Both `user.created` and `user.updated` use the same `upsert` logic — handling them identically makes retries and out-of-order delivery safe. `user.deleted` uses `deleteMany` so a duplicate delivery doesn't throw. Return codes follow Clerk's retry semantics: 4xx for malformed input (Clerk won't retry — the request is broken), 5xx for handler errors (Clerk will retry — transient failure).
